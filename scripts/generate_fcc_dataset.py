...
def rho_of_t(t_over_L: float, resolution: int) -> float:
    ...
    vox = gen_fcc(resolution=resolution, t_over_L=float(t_over_L))
...
def infer_t_range_from_rho(
    ...
    """scan for the t/L window whose FCC solid fraction sits in [rho_min, rho_max]"""
    ...
    if t_low >= t_high:
        raise ValueError(
            f"Inferred thickness interval empty: t_low={t_low:.6f}, "
            f"t_high={t_high:.6f}. "
...
def main():
    """run the FCC mech dataset generation end to end"""
    p = argparse.ArgumentParser(
        description="Generate FCC plate-lattice mechanical dataset via HomPy FEA."
    )
    ...
    p.add_argument("--n_candidates", type=int,   default=5000,
                   help="candidate thickness values to try (voxelised + hashed)")
    ...
    out_path = (Path(args.out) if args.out
                else Path(f"results/data/fcc_mech_res{args.resolution}.csv"))
    ...
    jitter = rng.normal(
        scale=(t_high - t_low) / (10 * args.n_candidates),
        size=args.n_candidates,
    )
    ...
    for t in candidates:
        ...
        vox = gen_fcc(resolution=args.resolution, t_over_L=t)
        rho = float(vox.mean())
        ...
        print(f"[{accepted:03d}/{args.n_target}] t/L={t:.5f}  rho={rho:.4f}  "
              f"C11={row['C11']:.4g}  time={dt:.1f}s  "
              f"(unique #{unique_found}, tried {tried})")
